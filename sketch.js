...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if(keyDown("space") && trex.collide(invisibleGround)|| touches.length>0) {
      trex.velocityY = -12;
      touches = [];
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    var cloud = createSprite(600,windowHeight/2,40,10);
    cloud.y = Math.round(random(windowHeight-100,windowHeight-300));
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(600,windowHeight-45,10,40);
    //obstacle.debug = true;
